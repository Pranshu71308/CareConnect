...
def admin_signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.set_password(form.cleaned_data['password'])  # Setting password manually
            user.save()

            # Ensure the group exists or is created
...
def discharge_patient_view(request, pk):
    ...
    days = (date.today() - patient.admitDate).days  # Calculate days directly
    assignedDoctor = models.User.objects.filter(id=patient.assignedDoctorId).first()  # Use .first() to get the first object or None

    if request.method == 'POST':
        # Calculate total and other fees
        total = (int(request.POST['roomCharge']) * days) + int(request.POST['doctorFee']) + int(request.POST['medicineCost']) + int(request.POST['OtherCharge'])

        # Create PatientDischargeDetails object
        pDD = models.PatientDischargeDetails.objects.create(
            patientId=pk,
            patientName=patient.get_name,
            assignedDoctorName=assignedDoctor.first_name if assignedDoctor else "",  # Handle case when assignedDoctor is None
...
def patient_dashboard_view(request):
    ...
    'admitDate':patient.admitDate,
     'timing1' :doctor.timing1,
    'timing2' : doctor.timing2,
    }
...
def contactus_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            from_email = 'pranshupancholi773@gmail.com'
            recipient_list = ['pranshupancholi77@gmail.com']
            send_mail(subject, message, from_email, recipient_list)
            
            return redirect('hospital/contactussuccess.html')  # Redirect to a success page
    else:
        form = FeedbackForm()
